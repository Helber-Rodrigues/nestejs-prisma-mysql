...
export class UserService {
    ...
    async update(id: number, { name, email, password, birthAt, role }: UpdatePutUserDto ) {
        ...
        if(password) password = await bcrypt.hash(password, await bcrypt.genSalt());
    ...
    async updatePartial(id: number, { name, email, password, birthAt, role }: UpdatePatchUserDto) {
        ...
        if(password) data.password = await bcrypt.hash(data.password, await bcrypt.genSalt());
...
function InjectRepositoy(UserEntity: any): (target: typeof UserService, propertyKey: undefined, parameterIndex: 0) => void {
